...
for _, row in view.iterrows():
    ...
    with st.expander(header, expanded=False):
        ...
        # BB 항목 — ETF는 신호 유효성 낮아 제외
        if not is_etf and _pct_b_now is not None:
            if _pct_b_now > 1.0:
                _msg = (f"가격이 볼린저밴드 상단을 뚫고 올라간 상태입니다. "
                        f"정상 변동 범위를 위로 벗어난 것으로, "
                        + ("알트코인 백테스트 기준 이 구간 이후 90일 평균 -11.7% — 단기 고점 가능성 있습니다."
                           if is_coin else "단기적으로 과열 구간에 진입했습니다."))
                _bear_items.append(("📊", _msg))
            elif _pct_b_now > 0.8:
                _bear_items.append(("📊",
                ...
                _bull_items.append(("📊",
                    f"가격이 볼린저밴드 하단을 이탈한 상태입니다. "
                    f"정상 범위를 아래로 벗어날 만큼 많이 내렸다는 뜻으로, "
                    + ("백테스트 기준 반등 확률 57%입니다." if is_coin else "기술적 반등 후보 구간입니다.")))
            elif _pct_b_now < 0.2:
                _bull_items.append(("📊",
