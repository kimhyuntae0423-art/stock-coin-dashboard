...
_SIGNAL_DISPLAY = [
    {
        ...
        "items_html": (
            "📦 ETF: 수익률 상위 50% 이내(Q1·Q2) — 비중 확대 또는 유지<br>"
            "📈 개별주: 최근 1년 수익률 상위 50% 이내 — 추세 유지 중<br>"
            "₿ BTC: 현재가가 역사적 평균 대비 저렴한 구간<br>"
            "₿ BTC: 과열 없이 가격이 강하게 상승하는 흐름<br>"
            "🪙 알트: 시장 전체가 저평가 구간이고 개별 손실 -20% 이내"
        ),
        "caption": "모멘텀 상위권일 때 🟢",
    },
    {
        ...
        "items_html": (
            ...
            "🪙 알트(G1 제외): 극단적 저점 + RSI도 바닥 — 종목별 승률 편차 큼"
            "(G2·G3 백테스트 43~79%, 미검증 신규 코인은 근거 없음)<br>"
    ...
    },
    {
        ...
        "items_html": (
            "📦 ETF: 수익률 하위 25% — 비중 축소 검토<br>"
            "📈 개별주: 매수가 대비 -8~20% 손실, 수익률 하위권,<br>"
            "&nbsp;&nbsp;또는 단기 과열 / -20% 손실인데 추세는 유지 중<br>"
            "₿ BTC: 시장 적정가를 넘어 과열 진입 전 구간<br>"
            "🪙 알트: 가격이 급등 후 꺾이는 반전 경보 (과거 -18.2%)"
        ),
        "caption": "-20% 손실 + 상승추세면 🟠 신호 충돌",
    },
    {
        "emoji": "🔴",
        "label": "매도 검토",
    ...
    },
    {
        ...
        "live_bg": "#e8f5e9", "live_bd": "#43a047",
        "items_html": (
            "코인 전용 — ETF·개별주 해당 없음<br>"
            "₿ BTC: 시장 평균보다 현재가가 낮은 극단적 저가 구간<br>"
            "&nbsp;&nbsp;과거 이 구간 매수 시 수익 확률 100%<br>"
            "🪙 알트: BTC 온체인 기준 동일하게 적용"
        ),
        "caption": None,
    },
]
...
for _, row in view.iterrows():
    ...
    with st.expander(header, expanded=False):
        ...
        # BB 항목 — ETF 제외, BTC/알트 신호 방향 분리
        if not is_etf and _pct_b_now is not None:
            if _pct_b_now > 1.0:
                if is_btc:
                    _bull_items.append(("📊",
                        "가격이 볼린저밴드 상단을 돌파했습니다. "
                        "BTC는 이 구간에서 상승이 이어지는 경향이 있습니다(백테스트 +23%, 81% 승률). "
                        "모멘텀 지속 여부를 확인하세요."))
                elif is_alt:
                    _bear_items.append(("📊",
                        "가격이 볼린저밴드 상단을 뚫고 올라간 상태입니다. "
                        "알트코인 백테스트 기준 이 구간 이후 90일 평균 -11.7% — 단기 고점 가능성 있습니다."))
                else:
                    _bear_items.append(("📊",
                        "가격이 볼린저밴드 상단을 이탈했습니다. 단기적으로 과열 구간에 진입했습니다."))
            elif _pct_b_now > 0.8:
                _bear_items.append(("📊",
                ...
                if is_btc:
                    _bear_items.append(("📊",
                        "가격이 볼린저밴드 하단을 이탈했습니다. "
                        "BTC는 이 구간에서 반등보다 추세 하락이 더 많았습니다. 주의가 필요합니다."))
                else:
                    _bull_items.append(("📊",
                        "가격이 볼린저밴드 하단을 이탈한 상태입니다. "
                        "정상 범위를 아래로 벗어날 만큼 많이 내렸다는 뜻으로, "
                        + ("알트코인 백테스트 기준 반등 확률 57%입니다." if is_alt
                           else "기술적 반등 후보 구간입니다.")))
            elif _pct_b_now < 0.2:
                _bull_items.append(("📊",
        ...
        if not is_etf and _bw_squeeze:
            _bear_items.append(("📊",
                "볼린저밴드 폭이 최근 들어 매우 좁아졌습니다. "
