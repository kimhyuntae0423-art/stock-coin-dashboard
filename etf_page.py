...
if "경기신호" in _macro:
    ...
    _mr_cols[_mr_idx].metric(
        ...
        _macro["경기신호"],
        cu_str,
        help="COPX 1개월 - GLD 1개월. 백테스트 검증(H17): IC=-0.37, p<0.001 — "
             "구리가 금 대비 강할수록 SPY 향후 1개월 수익은 오히려 낮았음 (역방향)."
    )
    _mr_idx += 1
...
if "곡선신호" in _macro:
    ...
    _mr_cols[_mr_idx].metric(
        ...
        _macro["곡선신호"],
        cv_str,
        help="장기채(TLT) vs 단기채(SHY) 1개월 성과. 백테스트 검증(H18): IC=+0.25, p<0.001 — "
             "안전자산 선호(장기채 우위)가 강할수록 SPY 향후 1개월 수익이 낮았음."
    )
    _mr_idx += 1
...
if "달러강도" in _macro:
    _mr_cols[_mr_idx % 4].metric(
        "달러 강도 (추정)",
        _macro["달러강도"],
        help="DXJ(일본 헤지) vs VEU(비미국) 상대강도. 백테스트 검증(H19): IC=+0.16, p=0.007 — "
             "강달러일수록 SPY 향후 1개월 수익이 높았음."
    )
