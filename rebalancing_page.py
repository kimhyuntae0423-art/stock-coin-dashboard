...
# ── 데이터 계산 ──────────────────────────────────────────────────────────
    ...
    if _e_alloc.empty:
        ...
        _rot_sum = pd.DataFrame([{
            ...
            "현재비중(%)": round(_rot_df["현재금액(원)"].sum() / max(_rot_total, 1) * 100, 1),
            "차이(%p)":   round(float(target_core + target_satellite + target_cash) - _rot_df["현재금액(원)"].sum() / max(_rot_total, 1) * 100, 1),
