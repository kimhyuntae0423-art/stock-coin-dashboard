...
if _i_mvrv < 0:
    _i_cards.append((
        ...
        f"MVRV Z {_i_mvrv:.2f} — 시장 평균 매수가보다 현재가가 낮은 구간",
        f"MVRV Z-Score는 비트코인 현재가가 시장 전체 평균 매수 원가 대비 "
        f"얼마나 고평가·저평가됐는지를 측정하는 온체인 지표입니다. "
        f"{_i_mvrv:.2f}는 시장 전체가 평균적으로 손실 상태라는 뜻으로, "
    ...
    _i_cards.append((
        ...
        f"MVRV Z {_i_mvrv:.2f} — 매도 트리거 미도달",
        f"MVRV Z-Score {_i_mvrv:.2f}는 역사적으로 BTC 강세장이 본격 시작되기 전 "
        f"바닥~초입 구간과 유사한 수준입니다. "
        ...
        f"MVRV가 1.5를 넘어갈 때부터 매도 액션을 시작하면 됩니다.",
    ))
elif _i_mvrv < 2.5:
    _i_cards.append((
        "🟠", "코인 사이클: 과열 경계 진입", "#fff7ed", "#f97316",
        f"MVRV Z {_i_mvrv:.2f} — 1단계 매도 트리거 활성화",
        f"MVRV Z {_i_mvrv:.2f}는 과거 BTC 사이클에서 '중간 과열' 구간에 해당합니다. "
        f"이 구간에서 BTC는 추가 상승 여지가 있었지만, "
    ...
    _i_cards.append((
        ...
        f"MVRV Z {_i_mvrv:.2f} — 역사적 고점 경계",
        f"MVRV Z {_i_mvrv:.2f}는 역사적으로 BTC 사이클 고점 근처에서 나타났던 수준입니다. "
