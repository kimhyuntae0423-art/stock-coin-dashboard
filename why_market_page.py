...
# ── 페이지 시작 ───────────────────────────────────────────
st.title("왜 시장을 사야 하는가")
st.markdown(
    "개별 종목 선택은 생각보다 훨씬 어렵습니다. "
    "우리 분석 대상 종목들의 실제 수익률 데이터로 직접 확인합니다."
)
    ...
    with col:
        st.markdown(
            ...
            <div style='background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px;
              ...
              <div style='font-size:13px; color:#64748b; margin-bottom:8px'>{label} ({len(sub)}개 종목)</div>
              ...
              <div style='font-size:13px; color:#64748b; margin-top:8px'>
                ...
                <span style='font-size:11px'>{beat}/{len(sub)}개</span>
              </div>
            </div>
            """,
    ...
    st.markdown(
        ...
        <div style='background:#fff1f2; border-left:4px solid #ef4444;
          ...
          <div style='font-size:13px; color:#555; margin-top:4px'>
            ...
            <span style='font-size:11px; color:#888'>(SPIVA 2024)</span>
          </div>
        </div>
        """, unsafe_allow_html=True)
with col2:
    ...
    st.markdown(
        ...
        <div style='background:#f0fdf4; border-left:4px solid #22c55e;
          ...
          <div style='font-size:13px; color:#555; margin-top:4px'>
            ...
            <span style='font-size:11px; color:#888'>나머지 37%로 알파 시도</span>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
# 섹션 5 — 결론
# ════════════════════════════════════════════════════════
st.subheader("우리 전략의 핵심 구조")
st.markdown(
    """
    | 버킷 | 비중 | 역할 | 근거 |
    |---|---|---|---|
    | **Core ETF** | 63% | 시장 평균 수익 확보 | 개별 종목의 60~70%가 시장 못 이김 |
    | **Satellite** | 15% | 알파 시도 (잃어도 치명적이지 않음) | 잘 고르면 아웃퍼폼 가능, 비중 제한 |
    | **BTC** | 15% | 비대칭 옵션 (4년 사이클) | 알트코인 대신 BTC만 — 알트 18%만 BTC 초과 |
    | **Cash** | 7% | 폭락 시 추가매수 탄약 | -15% 조정 시 Core 추가매수 |

    **핵심 행동 원칙**:
    - Core ETF는 매달 자동 매수 — 판단 개입 없음
    - 분기 리밸런싱으로 목표 비중 유지 — 자동 저가매수 효과
    - 주식/코인 신호는 현황 파악용 — 행동 근거로 쓰지 않음
    """
)
