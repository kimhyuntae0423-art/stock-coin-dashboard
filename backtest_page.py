...
# ════════════════════════════════════════════════════════
with tab_etf:
    st.subheader("📊 ETF 전략 백테스트 — 상대적 저점 Top 3")
    st.caption(
        "매월 말 H15(상대적 저점) 상위 3개 ETF 균등 보유 → 다음 달 반복.  \n"
        "벤치마크: VOO 동기간 매수보유. 매일 새벽 7시 자동 갱신. 수수료·슬리피지·세금 미반영."
    )
    ...
    if not _strat_eq_file.exists():
        ...
        st.caption(f"마지막 갱신: {_updated}  |  테스트 기간: {_eq_df['ym'].iloc[0]} ~ {_eq_df['ym'].iloc[-1]} ({len(_eq_df)}개월)")
...
with tab_val:
    ...
    # ── 6단계: H15/H16 — 검증 신호 복합 상대적 저점 지수 ──────────────────────
    st.markdown("### 6️⃣ H15/H16 — 상대적 저점 지수 (BB+MA 역방향 복합)")
    st.caption(
...
| **H15 상대적 저점** | 低BB+低MA→수익 높음 | +0.087 | <0.001 | ✅ 강력 유효 | ✅ 상대적 저점 순위 컬럼 (ETF/리밸런싱) |
| **BB 위치** | 상단→수익 낮음 | -0.087 | <0.001 | ✅ 역방향 유효 | ✅ 과열 페널티 + 상대적 저점 내 포함 |
| **MA 정렬** | 완전정렬→수익 낮음 | -0.065 | 0.002 | ✅ 역방향 유효 | ✅ 과열 페널티 + 상대적 저점 내 포함 |
...
""")
    st.success(
        "**신규 추가 (H15 상대적 저점)**: = -(BB위치)×0.57 - (MA정렬/3)×0.43  \n"
        "VIX 타이밍(언제 살지)과 결합하면: VIX>25 구간에서 상대적 저점 상위 ETF를 매수 — 두 검증 신호의 교집합."
    )
    st.error(
        "**중요**: 섹터사이클·Bull추세 필터·모멘텀 순위는 모두 백테스트 미검증.  \n"
        "배분의 실질 드라이버는 **VIX 국면**(강력 검증)과 **상대적 저점/과열 회피**(검증됨)입니다.  \n"
