...
def load_vs_market():
    ...
    def _ret(path):
        ...
        r["start"], r["end"], r["days"] = df.index[0], df.index[-1], days
        return r
    ...
    def _bench(ticker):
        ...
        r = compute_returns(float(close.iloc[0]), float(close.iloc[-1]), days)
        if r is None: return None, None
        return r["total_ret"], r["ann_ret"]
...
with tab_sum:
    ...
    def _load_sum():
        ...
        try:
            ...
            d["gc_hit"] = round(gc["hit_1M"].dropna().mean() * 100, 1) if len(gc) > 0 else None
            d["dc_hit"] = round(dc["hit_1M"].dropna().mean() * 100, 1) if len(dc) > 0 else None
        except Exception:
            d["gc_hit"], d["dc_hit"] = None, None
        try:
            rs = pd.read_csv(BACKTEST_DIR / "rsi_signals.csv")
            r70 = rs[rs["signal"] == "RSI 70 돌파(과매수)"]
            r30 = rs[rs["signal"] == "RSI 30 이탈(과매도)"]
            d["rsi70"] = round(r70["hit_22d"].dropna().mean() * 100, 1) if len(r70) > 0 else None
            d["rsi30"] = round(r30["hit_22d"].dropna().mean() * 100, 1) if len(r30) > 0 else None
        except Exception:
            d["rsi70"], d["rsi30"] = None, None
        try:
        ...
        try:
            ...
            d["mvrv_mdd"], d["btc_mdd"], d["mvrv_ann"] = None, None, None
        try:
        ...
        try:
            ...
            d["rb_ann"], d["rb_sharpe"], d["rb_mdd"] = None, None, None
        return d

    _s = _load_sum()

    def _fmt(val, spec="{:.1f}"):
        """CSV 로드 실패로 값이 None이면 가짜 숫자 대신 '확인 필요' 표시(CLAUDE.md 원칙4)."""
        return spec.format(val) if val is not None else "확인 필요"

    def _v(key, spec="{:.1f}"):
        return _fmt(_s.get(key), spec)
    ...
    with col_ok:
        st.success(
            "**✅ 검증된 신호 (적용 中)**\n\n"
            f"📈 12-1M 모멘텀 Q1 — 연 **{_v('mom_q1', '{:+.1f}%')}**\n\n"
            f"💎 퀄리티 ROE Q1 — 36M **{_v('qual_q1', '{:+.0f}%')}**\n\n"
            f"🔵 MVRV Z-Score — MDD **{_v('mvrv_mdd', '{:.0f}%')}** (BTC {_v('btc_mdd', '{:.0f}%')})\n\n"
            "📦 ETF 리밸런싱 — 샤프비율 우위"
        )
    with col_no:
        st.error(
            "**❌ 폐기 (제거/격하)**\n\n"
            f"🚫 골든크로스 — {_v('gc_hit', '{:.1f}%')} (동전던지기)\n\n"
            f"🚫 데스크로스 — {_v('dc_hit', '{:.1f}%')} (동전던지기)\n\n"
            f"🚫 RSI 70+ 과매수 (주식) — {_v('rsi70', '{:.1f}%')} (역방향)\n\n"
            "🚫 코인 모멘텀 Q1 — Q4가 더 높음"
        )
    with col_aux:
        st.warning(
            "**🟡 보조 신호 (참고만)**\n\n"
            f"⚠️ RSI 30 과매도 — {_v('rsi30', '{:.1f}%')} (단독 금지)\n\n"
            f"⚠️ 저변동성 — Q1 {_v('vol_q1', '{:.1f}%')}, Q4 {_v('vol_q4', '{:.1f}%')} (불마켓 역효과)\n\n"
    ...
    _coin_q_str = (
        f"Q1 {_v('coin_q1', '{:+.1f}%')} vs Q4 {_v('coin_q4', '{:+.1f}%')}"
        if _s.get("coin_q1") is not None else "Q4 > Q1 (역방향)"
    )
    _mom_diff  = (_s["mom_q1"] - _s["mom_q4"]) if _s.get("mom_q1") is not None and _s.get("mom_q4") is not None else None
    _rb_mdd_up = (24 + _s["rb_mdd"]) if _s.get("rb_mdd") is not None else None
    _mvrv_up   = abs(_s["mvrv_mdd"] - _s["btc_mdd"]) if _s.get("mvrv_mdd") is not None and _s.get("btc_mdd") is not None else None
    verdict_rows = [
        ("📈 주식 신호", "골든크로스 (50일>200일)",           f"1M 적중률 {_v('gc_hit', '{:.1f}%')}",                  "❌ 폐기",         "참고용 표시만",          "예측력 없음 — 동전던지기"),
        ("📈 주식 신호", "데스크로스 (50일<200일)",           f"1M 적중률 {_v('dc_hit', '{:.1f}%')}",                  "❌ 폐기",         "보조 참고만",            "예측력 없음 — 동전던지기"),
        ("📈 주식 신호", "RSI 70+ 과매수",                   f"22d 적중률 {_v('rsi70', '{:.1f}%')}",                  "❌ 역방향",       "주식 제거",              "과매수 후 오히려 계속 오르는 경향"),
        ("📈 주식 신호", "RSI 30 이하 과매도",               f"22d 적중률 {_v('rsi30', '{:.1f}%')}",                  "🟡 약한 우위",    "보조 신호 유지",         "56% 적중 — 단독 사용 금지"),
        ("🔬 팩터",      "12-1M 모멘텀 Q1",                  f"연 {_v('mom_q1', '{:+.1f}%')} (Q4 {_v('mom_q4', '{:+.1f}%')})", "✅ 검증됨", "우선순위 점수 주신호",   f"Q1 vs Q4 연 {_fmt(_mom_diff, '{:+.1f}%')}p"),
        ("🔬 팩터",      "퀄리티 ROE Q1",                    f"36M {_v('qual_q1', '{:+.0f}%')} vs Q4 {_v('qual_q4', '{:+.0f}%')}","✅ 검증됨", "QVGM 품질 점수 반영",   "고ROE 기업이 장기 우월"),
        ("🔬 팩터",      "저변동성 Q1",                      f"Q1 {_v('vol_q1', '{:+.1f}%')} vs Q4 {_v('vol_q4', '{:+.1f}%')}", "❌ 역방향(불마켓)","미적용",                "AI/테크 강세장에서 고변동성 압도"),
        ("📦 ETF 전략",  "VOO 단순보유 (Buy & Hold)",         "연 15-17%, MDD -24%",                                  "✅ 기준선",       "Core ETF 전략 근거",     "강세장에서 모든 전략 앞섬"),
        ("📦 ETF 전략",  "리밸런싱 60/30/10",                f"연 {_v('rb_ann', '{:.1f}%')}, 샤프 {_v('rb_sharpe', '{:.2f}')}", "🟡 MDD 제어", "전략 참고",              f"MDD {_fmt(_rb_mdd_up, '{:.1f}%')}p↓"),
        ("🪙 코인",      "BTC MVRV Z-Score 사이클",          f"MDD {_v('mvrv_mdd', '{:.0f}%')} vs BTC {_v('btc_mdd', '{:.0f}%')}","✅ 사이클 검증", "코인 페이지 1차 신호",  f"낙폭 {_fmt(_mvrv_up, '{:.0f}%')}p 절감"),
    ...
    with ins2:
        st.success(
            "**② 모멘텀과 퀄리티는 작동한다**\n\n"
            f"모멘텀 Q1 연 {_fmt(_mom_diff, '{:+.1f}%')}p 초과수익. "
    ...
    with st.expander("대시보드 변경 이력 (백테스트 기반)"):
        _history = pd.DataFrame([
            ("주식 추천",    "골든크로스 → 매수 신호",      "12-1M 모멘텀 Q1 → 주신호",     f"골든크로스 {_v('gc_hit', '{:.1f}%')}, 모멘텀 Q1 연 {_v('mom_q1', '{:+.1f}%')}"),
            ("주식 우선순위","action(골든크로스) 보정",      "mom_rank Q1~Q4 보정",          "Q1 +0.5, Q4 -0.3 백테스트 기반"),
            ("코인 추천",    "RSI 75+ 과매수 -1.0 패널티",  "RSI 75+ -0.2 (소폭만)",        f"주식 RSI70 {_v('rsi70', '{:.0f}%')} 적중 = 역방향 근거"),
            ("코인 페이지",  "MVRV 차트 아래 표시",          "MVRV 존 배너 최상단",          "가장 신뢰도 높은 1차 신호"),
            ("보유 종목",    "골든크로스 → 🟢 매수",         "모멘텀 Q1 → 🟢 매수",          f"골든크로스 {_v('gc_hit', '{:.1f}%')} vs 모멘텀 Q1 검증됨"),
            ("보유 종목",    "RSI 70+ → 🟠 주의",           "제거 (모멘텀 Q4로 교체)",       f"RSI 70+ 적중 {_v('rsi70', '{:.0f}%')} = 역효과"),
...
with tab_sig:
    ...
    with st.expander("✂️ 손절선 검증 (-8% / -20% 손절이 실제로 도움이 됐는가)"):
        ...
        with st.expander("종목별 손절선 상세"):
            # cut_*pct_better_to_cut는 결측(NaN)이 섞인 object dtype(True/False/NaN)이라
            # 그룹 내 값이 1개뿐이면 .sum()이 합산 없이 원본 bool을 그대로 반환해, 다른
            # 그룹의 int 합계와 섞여 pyarrow 직렬화(st.dataframe)가 깨졌다 — 2026-07-21 수정.
            def _safe_bool_sum(x):
            ...
            per = (loss_df.groupby("ticker").agg(
                ...
                cut8_도달=("cut_8pct_hit","sum"), cut8_유리=("cut_8pct_better_to_cut",_safe_bool_sum),
                cut20_도달=("cut_20pct_hit","sum"), cut20_유리=("cut_20pct_better_to_cut",_safe_bool_sum),
            ).reset_index())
