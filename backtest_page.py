...
with tab_val:
    ...
    # ── 6단계: H15/H16 — 검증 신호 복합 매수여건 지수 ────────────────────────
    st.markdown("### 6️⃣ H15/H16 — 매수여건 지수 (BB+MA 역방향 복합)")
    st.caption(
...
| **H15 매수여건** | 低BB+低MA→수익 높음 | +0.087 | <0.001 | ✅ 강력 유효 | ✅ 매수여건 순위 컬럼 (ETF/리밸런싱) |
| **BB 위치** | 상단→수익 낮음 | -0.087 | <0.001 | ✅ 역방향 유효 | ✅ 과열 페널티 + 매수여건 내 포함 |
| **MA 정렬** | 완전정렬→수익 낮음 | -0.065 | 0.002 | ✅ 역방향 유효 | ✅ 과열 페널티 + 매수여건 내 포함 |
...
""")
    st.success(
        "**신규 추가 (H15 매수여건)**: = -(BB위치)×0.57 - (MA정렬/3)×0.43  \n"
        "VIX 타이밍(언제 살지)과 결합하면: VIX>25 구간에서 매수여건 상위 ETF를 매수 — 두 검증 신호의 교집합."
    )
    st.error(
        "**중요**: 섹터사이클·Bull추세 필터·모멘텀 순위는 모두 백테스트 미검증.  \n"
        "배분의 실질 드라이버는 **VIX 국면**(강력 검증)과 **매수여건/과열 회피**(검증됨)입니다.  \n"
